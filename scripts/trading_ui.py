...
@media(min-width:761px){.detailgrid{grid-template-columns:minmax(0,1fr) minmax(300px,400px);align-items:start}.detailgrid>section,.detailgrid>aside,.detailgrid .card,.detailgrid .section{min-width:0}.detailgrid aside{position:sticky;top:20px;align-self:start}.decision-hero{grid-template-columns:minmax(200px,260px) minmax(0,1fr)}.verdict{max-width:100%;font-size:clamp(25px,2.4vw,34px);word-break:normal;overflow-wrap:normal;hyphens:none}.verdict-box{min-height:120px}.decision-summary,.report,pre,.mini-card,.resource-pill{min-width:0;overflow-wrap:anywhere}.report,pre{word-break:break-word}.detailgrid .btnrow{display:flex;grid-template-columns:none}.detailgrid .btn,.detailgrid button{width:auto}.tabs{min-width:0}.tab{white-space:normal}.chart-card,.chart-wrap{min-width:0;max-width:100%}.chart-wrap{overflow:hidden}.chart-svg{width:100%;max-width:100%;min-width:0}.detailgrid .table{table-layout:fixed;white-space:normal}.detailgrid .table th{width:32%;overflow-wrap:anywhere}.detailgrid .table td{min-width:0}.meta-value{display:block;max-width:100%;overflow-x:auto;white-space:nowrap;word-break:normal;overflow-wrap:normal}}
@media(max-width:760px){body{overflow-x:hidden;font-size:15px}main{width:100%;max-width:100vw;padding:16px 10px 48px;overflow-x:hidden}.hero{gap:10px;margin-bottom:16px;min-width:0}.hero>div{min-width:0;max-width:100%}.hero h1{font-size:26px;line-height:1.05;word-break:break-word;overflow-wrap:anywhere}.sub{word-break:break-word;overflow-wrap:anywhere}.detailgrid,.grid{display:block;min-width:0;max-width:100%}.detailgrid>section,.detailgrid aside,.card{min-width:0;width:100%;max-width:100%}.card{padding:14px;border-radius:16px;margin-bottom:14px;overflow:hidden}.decision-hero{display:block;padding:14px}.verdict-box{padding:14px;margin-bottom:12px;min-height:auto}.verdict{font-size:38px;line-height:1;word-break:normal;overflow-wrap:normal}.verdict-ticker{font-size:15px;word-break:break-word;overflow-wrap:anywhere}.decision-summary{font-size:15px;min-width:0}.decision-summary p,.decision-actions{overflow-wrap:anywhere;word-break:break-word}.btnrow{display:grid!important;grid-template-columns:minmax(0,1fr)!important;gap:8px;min-width:0;max-width:100%}.btn,.runbtn,button{width:100%;max-width:100%;min-height:48px;margin-top:8px;box-sizing:border-box}.tabs{display:grid!important;grid-template-columns:1fr!important;gap:7px}.tab{width:100%;padding:11px 8px}.report,pre{font-size:13px;line-height:1.5;white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;padding:12px;max-height:none}.report{max-height:none}.chart-controls{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:7px}.chart-range{padding:10px 6px;min-width:0}.legend{display:grid;grid-template-columns:1fr 1fr;gap:7px}.chart-wrap{overflow-x:auto;-webkit-overflow-scrolling:touch}.chart-svg{min-width:640px}.table{display:block;overflow-x:auto;white-space:normal}.table tbody,.table tr,.table th,.table td{display:block;width:100%;max-width:100%}.table tr{border-bottom:1px solid #243044;padding:8px 0}.table th,.table td{border-bottom:0;padding:4px 0;word-break:break-word;overflow-wrap:anywhere}.meta-value{white-space:normal;overflow-x:visible;overflow-wrap:anywhere;word-break:break-word}.mini-grid{grid-template-columns:1fr!important}.mini-value{font-size:15px;overflow-wrap:anywhere}.decision{padding-left:10px;margin:10px 0}.resource-list{gap:6px}.resource-pill{font-size:12px;max-width:100%;overflow-wrap:anywhere}.chart-summary{font-size:14px}.section{min-width:0;max-width:100%}}
...
def period_for_range(range_key: str) -> tuple[str, int, str]:
    """Return (yfinance_period, keep_rows, yfinance_interval)."""
    mapping = {
        "1d": ("5d", 27, "15m"),
        "1w": ("1mo", 40, "1h"),
        "1m": ("1y", 31, "1d"),
    ...
    return mapping.get(range_key, mapping["1m"])
...
def get_chart_data(ticker: str, range_key: str, verdict: str) -> dict[str, object]:
    ...
    period, keep_days, interval = period_for_range(range_key)
    try:
        ...
        date_format = "%Y-%m-%d %H:%M" if interval in {"15m", "1h", "5m", "30m"} else "%Y-%m-%d"
        for idx, row in sliced.iterrows():
            item: dict[str, object] = {"date": idx.strftime(date_format)}
...
function renderHistory(runs){{ if(!runs.length){{historyBox.innerHTML='<p class="muted">No runs yet.</p>';return;}} historyBox.innerHTML='<table class="table"><thead><tr><th>Run</th><th>Decision</th><th>Status</th><th>Model</th><th>Created</th><th></th></tr></thead><tbody>'+runs.map(r=>`<tr><td><strong>${{escapeHtml(r.ticker)}} / ${{escapeHtml(r.date)}}</strong><br><span class="tiny">${{escapeHtml(r.id)}}</span></td><td><span class="pill" style="border-color:${{r.tone==='buy'?'#46d17d':r.tone==='sell'?'#ff6b6b':r.tone==='hold'?'#d9a441':'#334158'}};color:${{r.tone==='buy'?'#8dffb7':r.tone==='sell'?'#ff9b9b':r.tone==='hold'?'#ffd47a':'#91a0b7'}};background:${{r.tone==='buy'?'rgba(70,209,125,.12)':r.tone==='sell'?'rgba(255,107,107,.12)':r.tone==='hold'?'rgba(217,164,65,.12)':'#0a0f17'}}">${{escapeHtml(r.verdict||'—')}}</span></td><td><span class="pill">${{escapeHtml(r.status)}}</span></td><td>${{escapeHtml(r.model||'')}}</td><td>${{escapeHtml(r.created_at||'')}}</td><td><a class="btn secondary" href="/run/${{encodeURIComponent(r.id)}}">Open</a></td></tr>`).join('')+'</tbody></table>';}}
...
loadHistory();
// --- Chart drawing (reused from detail page) ---
...
function drawChart(target,data){{
  ...
  const last=rows[rows.length-1], lx=x(rows.length-1), ly=yP(last.close), c=data.verdict?toneColor(data.verdict):'#d9a441';
  ...
  const verdictLabel=data.verdict?`<text x="${{W-pad}}" y="24" fill="${{c}}" font-size="13" font-weight="900" text-anchor="end">${{esc(data.verdict)}}</text>`:'';
  const labels=`<text x="${{pad}}" y="24" fill="#91a0b7" font-size="12">${{esc(data.ticker)}} ${{esc(data.range)}} · latest $${{Number(last.close).toFixed(2)}}</text>${{verdictLabel}}<text x="${{pad}}" y="${{rsiTop-8}}" fill="#91a0b7" font-size="12">RSI</text><text x="${{pad}}" y="${{volTop-8}}" fill="#91a0b7" font-size="12">Volume</text><text x="${{pad}}" y="${{H-12}}" fill="#738198" font-size="11">${{esc(rows[0].date)}}</text><text x="${{W-pad}}" y="${{H-12}}" fill="#738198" font-size="11" text-anchor="end">${{esc(last.date)}}</text>`;
  el.innerHTML=`<svg class="chart-svg" viewBox="0 0 ${{W}} ${{H}}" role="img" aria-label="${{esc(data.ticker)}} price chart">${{grid}}<polyline points="${{closePts}}" fill="none" stroke="#edf2ff" stroke-width="2.4"/><polyline points="${{ma20}}" fill="none" stroke="#46d17d" stroke-width="1.4" opacity=".9"/><polyline points="${{ma50}}" fill="none" stroke="#ffd47a" stroke-width="1.4" opacity=".9"/><polyline points="${{ma200}}" fill="none" stroke="#ff6b6b" stroke-width="1.4" opacity=".9"/><line x1="${{pad}}" x2="${{W-pad}}" y1="${{yR(70)}}" y2="${{yR(70)}}" stroke="#8a6521" stroke-dasharray="5 5"/><line x1="${{pad}}" x2="${{W-pad}}" y1="${{yR(30)}}" y2="${{yR(30)}}" stroke="#276f43" stroke-dasharray="5 5"/><polyline points="${{rsi}}" fill="none" stroke="#7aa2ff" stroke-width="1.8"/>${{bars}}<circle cx="${{lx}}" cy="${{ly}}" r="5" fill="#d9a441" stroke="#05070b" stroke-width="2"/>${{labels}}</svg>`;
  const sum=document.getElementById('portfolio-summary'); if(sum && data.summary){{sum.style.display='block'; sum.textContent=data.summary;}} else if(sum){{sum.style.display='none';}}
}}
// --- Portfolio interaction ---
let activeTicker='';
let activeRange='1m';
const ptickers=document.querySelectorAll('.portfolio-ticker');
const pranges=document.querySelectorAll('#portfolioRanges .chart-range');
async function loadPortfolioChart(ticker,range){{
  const sum=document.getElementById('portfolio-summary'); if(sum){{sum.style.display='block'; sum.textContent='Loading '+ticker+' chart…';}}
  const res=await fetch('/api/portfolio/chart?ticker='+encodeURIComponent(ticker)+'&range='+encodeURIComponent(range));
  const data=await res.json();
  drawChart('portfolio-chart', data);
}}
ptickers.forEach(btn=>btn.onclick=()=>{{
  activeTicker=btn.dataset.ticker;
  ptickers.forEach(b=>b.classList.remove('on'));
  btn.classList.add('on');
  loadPortfolioChart(activeTicker, activeRange);
}});
pranges.forEach(btn=>btn.onclick=()=>{{
  activeRange=btn.dataset.range;
  pranges.forEach(b=>b.classList.remove('active'));
  btn.classList.add('active');
  if(activeTicker) loadPortfolioChart(activeTicker, activeRange);
}});
</script></body></html>"""
...
def detail_page(run_id: str) -> bytes:
    ...
    meta_rows = "".join(f'<tr><th>{html_escape(k)}</th><td><span class="meta-value">{html_escape(v)}</span></td></tr>' for k, v in meta.items() if k not in {"decision"})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path == "/api/runs":
            ...
            for run in runs:
                r = dict(run)
                snap = decision_snapshot(r.get("decision", ""), r.get("ticker", ""))
                r["verdict"] = snap["verdict"]
                r["tone"] = snap["tone"]
                enriched.append(r)
            self.json(200, enriched)
            return
        if path == "/api/portfolio/chart":
            query = parse_qs(parsed.query)
            ticker = query.get("ticker", [""])[0].upper()
            range_key = query.get("range", ["1m"])[0]
            if not ticker or not TICKER_RE.match(ticker):
                self.json(400, {"error": "invalid ticker"})
                return
            self.json(200, get_chart_data(ticker, range_key, ""))
            return
